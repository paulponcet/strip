...
test_that("predict.lm works correctly after stripping", {
  ...
  p2 = try(predict(r2, newdata = mtcars[i == 2,]), silent = TRUE)
...
test_that("predict.glm works correctly after stripping", {
  ...
  ld = seq(0, 5, 0.1)
  r1 = glm(SF ~ sex*ldose, family = binomial)
  r2 = strip(r1, keep = "predict")
  df = data.frame(ldose = ld,
                  sex = factor(rep("M", length(ld)), levels = levels(sex)))
  p1 = predict(r1, newdata = df, type = "response")
  p2 = try(predict(r2, newdata = df, type = "response"), silent = TRUE)
...
test_that("predict.loess works correctly after stripping", {
  ...
  r1 = loess(dist ~ speed, cars[i == 1,])
  r2 = strip(r1, keep = "predict")
  p1 = predict(r1, newdata = cars[i == 2,])
  p2 = try(predict(r2, newdata = cars[i == 2,]), silent = TRUE)
...
test_that("predict.randomForest works correctly after stripping", {
  ...
  r1 = randomForest(Species ~ ., data = iris[i == 1,])
  r2 = strip(r1, keep = "predict")
  p1 = predict(r1, newdata = iris[i == 2,])
  p2 = try(predict(r2, newdata = iris[i == 2,]), silent = TRUE)
...
test_that("print.randomForest works correctly after stripping", {
  require("randomForest")
  data("iris")
  r1 = randomForest(Species ~ ., data = iris)
  r2 = strip(r1, keep = "print")
  q1 = print(r1)
  q2 = try(print(r2), silent = TRUE)
...
test_that("predict.train works correctly after stripping", {
  ...
  set.seed(311)
  i = sample(2, nrow(iris), replace = TRUE, prob=c(0.8, 0.2))
  r1 = train(Species ~ ., data = iris[i == 1,], method = "rf")
  r2 = strip(r1, keep = "predict")
  p1 = predict(r1, newdata = iris[i == 2,])
  p2 = try(predict(r2, newdata = iris[i == 2,]), silent = TRUE)
...
test_that("print.train works correctly after stripping", {
  require("caret")
  data("iris")
  r1 = train(Species ~ ., data = iris, method = "rf")
  r2 = strip(r1, keep = "print")
  q1 = print(r1)
  q2 = try(print(r2), silent = TRUE)
